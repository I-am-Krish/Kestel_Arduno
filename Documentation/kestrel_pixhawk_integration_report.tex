...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        esl/.style={rectangle, draw=darkblue, thick,
                    ...
                    text width=8.3cm, rounded corners=2pt},
        %% PC GCS stage box
    ...
    \draw[->, skyblue, line width=2.8pt, >=stealth]
        (s6.east) -- (p1.west);
    \node[below, font=\footnotesize, text=skyblue, align=center] at (9.0, -0.4)
        {USB Micro-B cable $\cdot$ 115{,}200 baud\\[1pt] \texttt{/dev/ttyUSB0}};
    \node[below, font=\tiny, text=codegray, align=center] at (9.0, -1.0)
        {encrypted Kestrel binary packets};

    %% ==================================================================
    %% PC GCS COLUMN  (x = 18.0, top of column = USB RX at y = 0)
    %% ==================================================================
    \node[font=\normalsize\bfseries, text=successgreen] at (18.0, 1.0)
        {PC Ground Control Station};
    \node[font=\tiny, text=codegray] at (18.0, 0.6)
        {kestrel\_gcs\_usb.py (Python 3.10)};

    \node[usbl] (p1) at (18.0, 0)
        {\textbf{USB Serial Receive}\\[2pt]
         \texttt{pyserial} \texttt{serial.read(64)}\\[1pt]
         \texttt{/dev/ttyUSB0} @ 115{,}200 baud};

    \node[pcl] (p2) at (18.0, -1.6)
    ...
    \node[pcl, fill=yellow!8, draw=warnorange] (p5) at (18.0, -6.4)
    ...
    \node[anno, text width=2.9cm, align=right] at (12.0, -0.8) {Received raw bytes\\(encrypted Kestrel)};
    \node[anno, text width=2.9cm, align=right] at (12.0, -2.4) {Validated Kestrel\\frame buffer};
    \node[anno, text width=2.9cm, align=right] at (12.0, -4.0) {Authenticated\\plaintext payload};
    \node[anno, text width=2.9cm, align=right] at (12.0, -5.6) {Decoded message\\fields};
