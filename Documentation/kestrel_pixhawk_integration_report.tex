...
\begin{longtable}{@{}p{1.2cm}p{2.8cm}p{3.2cm}p{6.5cm}@{}}
    \toprule
    ...
    0.1 & Jun 22, 2026 & Krishnashis Das &
        Initial wiring and firmware upload. First ESP32 bridge sketch flashed. \\
    \midrule
    0.5 & Jun 22, 2026 & Krishnashis Das &
        Parser CRC fix and MAV\_TYPE table correction. Serial link verified
        (27 packets, 0 errors). \\
    \midrule
    0.8 & Jun 22, 2026 & Krishnashis Das &
        SR0/SR1 stream rate parameters set on Pixhawk; SERIAL1 protocol
        confirmed. All telemetry streams verified over USB. \\
    \midrule
    1.0 & Jun 23, 2026 & Krishnashis Das &
        Final live hardware validation. 378 Kestrel packets received,
        ATT:209 GPS:85 HB:84 with 0 errors. Document finalised. \\
\end{longtable}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{}lXl@{}}
        ...
        CP2102 USB-UART Bridge & USB to UART converter embedded on ESP32 board &
            USB link to PC \\
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
        box/.style={rectangle, draw=darkblue, very thick, minimum width=3.2cm,
        ...
        \node[box, fill=blue!10] (px) at (0, 0)
            {\textbf{Pixhawk}\\CubeOrange+\\ArduCopter 4.x};

        % ESP32
        \node[box, fill=orange!10] (esp) at (6, 0)
            {\textbf{ESP32}\\Bridge Firmware\\KestrelPixhawkBridge};

        % PC
        \node[box, fill=green!10] (pc) at (12, 0)
            {\textbf{PC (Linux)}\\Kestrel GCS\\kestrel\_gcs\_usb.py};

        % Arrows
        \draw[arrow] (px.east) -- node[above, font=\footnotesize]
            {MAVLink v2} node[below, font=\tiny]{UART @ 57600 baud} (esp.west);
        \draw[arrow] (esp.east) -- node[above, font=\footnotesize]
            {Kestrel + ChaCha20} node[below, font=\tiny]{USB @ 115200 baud}
            (pc.west);

        % Labels below
        \node[font=\tiny, below=0.6cm of px] {TELEM1 (GPIO TX/RX)};
        \node[font=\tiny, below=0.6cm of esp] {GPIO16/17 $\leftrightarrow$ USB};
        \node[font=\tiny, below=0.6cm of pc] {/dev/ttyUSB0};

    \end{tikzpicture}
    \caption{System Communication Topology: Pixhawk $\rightarrow$ ESP32 Bridge
    $\rightarrow$ PC GCS}
\end{figure}
